"""Healthy scenario: one service, every capability configured, nothing unusual."""
    ...
    logs_provider = FakeLogsProvider(
        {
            _LOGS_REF: [
                ...
                LogEvent(
                    ...
                    service=_SERVICE,
                    environment=_ENVIRONMENT,
                    message="request completed in 38ms",
                    source="app",
                    provider="fake_logs",
                ),
            ]
    ...
    metrics_provider = FakeMetricsProvider(
        {
            _METRICS_REF: {
                "RequestCount": RawMetricSeries(
                    ...
                    unit="count",
                    points=[
                        MetricPoint(timestamp=_at(11, 30), value=120.0),
                        MetricPoint(timestamp=_at(11, 45), value=118.0),
                        MetricPoint(timestamp=_at(11, 55), value=125.0),
                    ],
                ),
                "ErrorRate": RawMetricSeries(
                    ...
                    unit="percent",
                    points=[
                        MetricPoint(timestamp=_at(11, 30), value=0.4),
                        MetricPoint(timestamp=_at(11, 45), value=0.5),
                        MetricPoint(timestamp=_at(11, 55), value=0.3),
                    ],
                ),
                "LatencyP99": RawMetricSeries(
                    ...
                    unit="milliseconds",
                    points=[
                        MetricPoint(timestamp=_at(11, 30), value=95.0),
                        MetricPoint(timestamp=_at(11, 45), value=98.0),
                        MetricPoint(timestamp=_at(11, 55), value=91.0),
                    ],
                ),
            }
        }
    ...
    health_provider = FakeHealthProvider(
        {
            _HEALTH_REF: HealthSignal(
                service=_SERVICE,
                environment=_ENVIRONMENT,
                reported_status=HealthStatus.HEALTHY,
